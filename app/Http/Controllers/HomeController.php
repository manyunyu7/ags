...
//        return view('home.admin');
    ...
    public function eval(Request $request){
        ...
        if ($case=="1"){
            return view("learn.05");
        }

        if ($case=="2"){
            return view("learn.06");
        }

        if ($case=="3"){
            return view("learn.07");
        }
    }
    ...
    public function evalResult(Request $request){

        $data = QuizAnswe::where("user_id",'=',Auth::id())->first();
        return view("learn.11")->with(compact('data'));
    }
